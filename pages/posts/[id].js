		...
		<React.Fragment>
			...
			<Container className={classes.blogContainer} maxWidth="md">
                <Box component="div" className={classes.introBlock}>
                    <Avatar alt="Author's image" className={classes.introAvatar}>r</Avatar>
                ...
                <Typography variant="h2" component="h1" gutterBottom className={classes.articleTitle}>
                    {post.title}
                </Typography>

                <Typography variant="subtitle1" gutterBottom className={classes.articleDetails}>
                ...
                <Typography variant="h6" component="h6" className={classes.postSuggestions}>
					{/* Suggested Posts */}
					<Divider />
				</Typography>
				<Copyright />
...
export async function getStaticProps({ params }) {
	const res = await fetch(
        `https://kh-blog-app.herokuapp.com/api/v1/articles/${params.id}`
    );
...
export async function getStaticPaths() {
	...
	return {
		paths: new Array(getPath.length).fill(null).map((_, index) => (
            { params: getPath[index] }
        )),
		fallback: true,
	};
}
